(ns chemmineops.chemmineopsllm
  ...
  *proposal* (with a rationale + the fields it cited), never a
  ...
  censored downstream by `chemmineops.governor` before anything touches
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [chemmineops.facts :as facts]
            [chemmineops.registry :as registry]
            [chemmineops.robotics :as robotics]
...
(defn- assess-jurisdiction
  ...
  spec-basis in `chemmineops.facts` -- the Quarry Governor must reject
  ...
  (let [e (store/extraction db subject)
    ...
    (if (nil? sb)
      {:summary    (str iso3 " の公式spec-basisが見つかりません")
       :rationale  "chemmineops.facts に未登録の法域。要件を推測で作らない。"
       :cites      []
...
(defn- simulate-quarry-face-verification
  ...
  (`chemmineops.robotics`) and drafts its result as a proposal. High
  ...
  `chemmineops.governor`'s `robotics-simulation-violations`."
  [db {:keys [subject]}]
  (let [e (store/extraction db subject)]
    (if (nil? e)
...
(defn- propose-extraction
  ...
  `:auto` set (`chemmineops.phase`); the governor also always escalates
...
(defn- propose-shipment
  ...
  set (`chemmineops.phase`); the governor also always escalates on
