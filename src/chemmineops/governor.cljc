(ns chemmineops.governor
  ...
  `:itonami.blueprint/governor` is `:chem-mineral-governor`, grep-verified
  ...
  human may approve -- but see `chemmineops.phase`: for `:stake
  ...
  bug (ADR-2607071320)."
  (:require [chemmineops.facts :as facts]
...
(defn- robotics-simulation-violations
  ...
  quarry-face verification mission (`chemmineops.robotics`) never ran
  ...
  face-boundary-deviation fields (`chemmineops.robotics/simulation-out-
...
(defn- royalty-mismatch-violations
  ...
  `chemmineops.registry/royalty-matches-claim?` -- needs no proposal
