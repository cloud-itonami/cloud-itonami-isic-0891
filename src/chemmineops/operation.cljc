(ns chemmineops.operation
  ...
  this node when the governor is clean -- see `chemmineops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [chemmineops.chemmineopsllm :as chemmineopsllm]
            [chemmineops.governor :as governor]
...
(defn build
  ...
  `chemmineops.store/Store`).
  opts:
    :advisor      -- a `chemmineops.chemmineopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (chemmineopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (chemmineopsllm/-advise advisor store request)]
            {:proposal p :audit [(chemmineopsllm/trace request p)]})))
